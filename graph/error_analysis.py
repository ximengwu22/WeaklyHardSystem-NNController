...
def bernstein_error_partition_cuda(
    ...
    fo = open("result/BPres-"+filename+".txt", "a+")
    fo.write('---------------' + filename + '-------------------'+"\n")
    fo.write('step: {}'.format(step)+"\n")
    fo.write('degree bound: {}'.format(degree_bound)+"\n")
    fo.write('number of partition: {}'.format(num_partition)+"\n")
    fo.write('Lipschitz constant: {}'.format(lips)+"\n")
    fo.close()
...
if __name__ == '__main__':
    ...
    run(args)
    actv = 'ReLU'
    nn = nn_controller_details(args.filename, actv, reuse=True)
    x = sp.symbols('x:' + str(nn.num_of_inputs))

    d = [3,3]
    box = [[0.2,0.9],[-0.3,0.5]]  # ODE 1
    output_i = 0
