...
vector<vector<Expression_AST<Real>>> uexpr;
...
void buildOneStepGraph() {
    ...
    for (int start = 0; start < grids.size(); start++) {
        ...
        for (int meet = 0; meet < 2; meet++) {
            ...
            // ------------------------------------------------
            // TODO: get grid x,y, call bernstein_poly_approx
            // ------------------------------------------------
    ...
    printf("[Success] Number of edges in Graph: %d\n", edgeCnt);
}
...
void buildKStepGraph() {
    ...
    printf("          End Region: %d\n", Tk.count());
    printf("          Number of Edges in Kstep: %d\n", edge);
    //[add]
//    printf("          UNSAFE REGION: %d\n", );
    //[end]
}
